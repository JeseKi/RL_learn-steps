...
def train(agents: List[GreedyAgent], steps: int) -> Tuple[List[GreedyAgent], Rewards, AverageMetrics]:
...
def _round(agent: GreedyAgent, steps: int):
    ...
    for i in range(steps):
        ...
        if agent.episode_state.epsilon <= 0.5 and not _printed[0]:
            ...
            _printed[0] = True
        if agent.episode_state.epsilon <= agent.episode_state.min_epsilon and not _printed[1]:
...
def _average_rewards(rewards_list: List[Rewards]) -> Rewards:
    ...
    avg_values: List[float] = [sum(r.values[i] for r in rewards_list) / num_agents for i in range(num_machines)]
    avg_counts: List[float] = [sum(r.counts[i] for r in rewards_list) / num_agents for i in range(num_machines)]
...
def _average_metrics_structured(agents: List[GreedyAgent]) -> AverageMetrics:
    ...
    return AverageMetrics(avg_regret=avg_regret,avg_regret_rate=avg_regret_rate, avg_total_reward=avg_total_reward, avg_optimal_rate=avg_optimal_rate)