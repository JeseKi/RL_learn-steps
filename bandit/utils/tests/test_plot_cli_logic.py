...
def make_run_with_duplicates(
    agent: str, steps, values_a: dict, values_b: dict
) -> ProcessRun:
        ...
        points.append(
            ProcessPoint(
                ...
                data=ProcessPointData(
                    ...
                    total_reward=float(
                        values_a.get("total_reward", [0] * len(steps))[i]
                    ),
                    optimal_rate=float(
                        values_a.get("optimal_rate", [0] * len(steps))[i]
                    ),
                    convergence_steps=0,
                ),
            )
        ...
        points.append(
            ProcessPoint(
                ...
                data=ProcessPointData(
                    ...
                    total_reward=float(
                        values_b.get("total_reward", [0] * len(steps))[i]
                    ),
                    optimal_rate=float(
                        values_b.get("optimal_rate", [0] * len(steps))[i]
                    ),
                    convergence_steps=0,
                ),
            )
...
def test_find_pairwise_intersections_for_metric():
    ...
    crosses = find_pairwise_intersections_for_metric(
        steps, {"A": y1, "B": y2}, min_gap_ratio=0.05
    )
